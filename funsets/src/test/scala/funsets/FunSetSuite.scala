...
class FunSetSuite extends FunSuite {
  ...
  test("intersection contains only elements which are in both sets") {
    new TestSets {
      val s = intersect(_ < 5, _ > 2)
      assert(contains(s, 3), "Intersect 1")
      assert(!contains(s, 2), "Intersect 2")
      assert(contains(s, 4), "Intersect 3")
    }
  }
  test("diff contains only elements from first set which are not in second") {
    new TestSets {
      val s = diff(_ < 5, _ > 2)
  ...
  test("filter contains only elements which are true from predicate") {
    new TestSets {
      val s = filter(_ < 10, _ > 5)
